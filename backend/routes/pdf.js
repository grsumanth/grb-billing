...
async function generateBillPDF(bill, items) {
  return new Promise((resolve, reject) => {
    ...
    items.forEach((item, i) => {
      ...
      doc.text(item.product_name, 30, y, { width: W * 0.48, ellipsis: true });
      doc.text(item.type || '', W * 0.52, y, { width: W * 0.12 });
      doc.text(String(item.quantity), W * 0.65, y, { width: 40, align: 'center' });
      doc.text('Rs.' + parseFloat(item.price).toFixed(2), W * 0.76, y, { width: 45, align: 'right' });
      doc.fillColor(burgundy).font('Helvetica-Bold')
        .text('Rs.' + parseFloat(item.total).toFixed(2), W * 0.87, y, { width: W * 0.13 - 10, align: 'right' });
      y += 20;
    });
